...
Future<Marketplace1ItemPage> fetchPage(int startingIndex) async {
  ...
  var response = await http.get(Uri.parse(nig_hostname+'/marketplace_step/1/'+user_public_key_hash+'/'+sell_amount.toString()));
    ...
    for (var elem in fetched_results.results) {
      if (elem['readonly_flag']==false){
        ...
        fetched_marketplace1_list.add(Marketplace1Item(
          color: color,
          ...
          buyer_reput_reliability: elem['buyer_reput_reliability'].toDouble(),
        ));
        }
    };
