...
import 'purchase_followup_page.dart';
...
Future<Marketplace2ItemPage> fetchPage(int startingIndex) async {
  ...
  List<Marketplace2Item> fetched_marketplace2_list =[] ;
  print("user_public_key_hash");
  print(user_public_key_hash);
  var response = await http.get(Uri.parse(nig_hostname+'/marketplace_step/2/'+user_public_key_hash));
    ...
    for (var elem in fetched_results.results) {
      ...
      fetched_marketplace2_list.add(Marketplace2Item(
        color: color,
